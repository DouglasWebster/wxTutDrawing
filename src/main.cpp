...
void MyFrame::OnAddButtonClick(wxCommandEvent& event)
{
    ...
    std::uniform_int_distribution colourDistrib(0, 0xFFFFFF);

    rectCount++;
    canvas->addRect(sizeDistrib(randomGen), sizeDistrib(randomGen), xDistrib(randomGen), yDistrib(randomGen), angleDistrib(randomGen),
        wxColour(colourDistrib(randomGen)), "Rect #" + std::to_string(rectCount));
}
...
{
    SetStatusText("Rect named " + event.GetString() + " added!");
}

void MyFrame::OnRectRemoved(wxCommandEvent& event)
{
    SetStatusText("Rect named " + event.GetString() + " removed!");
}
